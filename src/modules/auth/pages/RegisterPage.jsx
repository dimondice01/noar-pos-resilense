...
import { 
    ...
    PartyPopper // Nuevo ícono para la fiesta
} from 'lucide-react';
...
import confetti from 'canvas-confetti'; // 🔥 IMPORTAMOS CONFETI
...
export const RegisterPage = () => {
    ...
    return (
        <div className="min-h-screen flex bg-sys-50">
            ...
            {/* IZQUIERDA: Branding */}
            <div className="hidden lg:flex w-1/2 bg-sys-900 text-white flex-col justify-between p-12 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-96 h-96 bg-brand/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3"></div>
                ...
                <div className="z-10">
                    ...
                    <p className="mt-2 text-sys-400">Resilience System v2.0</p>
                </div>

                <div className="z-10 max-w-md">
                    <h2 className="text-4xl font-bold mb-6 leading-tight">
                        Tu negocio merece <span className="text-brand">tecnología de punta</span>.
                    </h2>
                    <div className="space-y-4 text-sys-200">
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="text-brand" /> <span>Facturación Ilimitada</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="text-brand" /> <span>Sin Costos de Instalación</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <CheckCircle2 className="text-brand" /> <span>Soporte 24/7</span>
                        </div>
                    </div>
                </div>

                <div className="text-xs text-sys-500 z-10">
                    © 2025 Noar Technology.
                </div>
            </div>
                    ...
                    {step === 1 && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <div className="text-center mb-6">
                                <h2 className="text-3xl font-bold text-sys-900">Crear Cuenta</h2>
                                <p className="text-sys-500 mt-2">Prueba gratuita de 3 días. Sin tarjeta.</p>
                            </div>
                            ...
                            <form onSubmit={handleRegister} className="space-y-4">
                                        ...
                                        <div className={cn(
                                            ...
                                            {logoPreview ? (
                                                ...
                                                <div className="text-center text-sys-400">
                                                    ...
                                                    <span className="text-[9px] font-bold uppercase">Tu Logo</span>
                                                </div>
                                            )}
                                        </div>
                                        <input 
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold uppercase text-sys-500 ml-1">Tu Nombre</label>
                                ...
                                <div className="bg-sys-100 p-3 rounded-xl border border-sys-200 flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-sm text-sys-600">
                                        ...
                                        <span>¿Cuánto es <strong>{mathChallenge.q}</strong>?</span>
                                    </div>
                                ...
                                <Button type="submit" disabled={loading} className="w-full py-4 text-base shadow-xl shadow-brand/20 bg-sys-900 hover:bg-black text-white transition-all hover:scale-[1.02]">
                                    {loading ? <Loader2 className="animate-spin"/> : <>Crear Tienda <ArrowRight size={18} className="ml-2"/></>}
                                </Button>
                            </form>
                            
                            <p className="text-center text-xs text-sys-400 mt-6">
                                ¿Ya tienes cuenta? <span className="text-brand font-bold cursor-pointer hover:underline" onClick={() => navigate('/login')}>Iniciar Sesión</span>
                            </p>
                        </div>
                    )}
                    ...
                    {step === 2 && (
                        <div className="animate-in fade-in slide-in-from-right-8 duration-500">
                            <div className="text-center mb-8">
                                ...
                                <h2 className="text-2xl font-bold text-sys-900">¡Bienvenido, {formData.ownerName}!</h2>
                                <p className="text-sys-500 mt-2">Personalicemos tu catálogo inicial.</p>
                            </div>
                            ...
                            {isSeeding ? (
                                <div className="text-center py-12 bg-white rounded-2xl border border-sys-200 shadow-lg px-6 relative overflow-hidden">
                                    ...
                                    <h3 className="text-lg font-bold text-sys-800">Cargando Productos...</h3>
                                    <p className="text-sm text-sys-500 mt-2 font-medium animate-pulse">{loadingMsg}</p>
                                    <p className="text-xs text-sys-400 mt-6">Esto toma unos segundos. No cierres la ventana.</p>
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    ...
                                    <Button onClick={handleFinish} disabled={!selectedOption} className="w-full py-4 mt-6 text-base bg-brand hover:bg-brand-dark text-white shadow-xl shadow-brand/20 transition-all hover:-translate-y-1">
                                        {selectedOption?.file ? "Cargar Productos y Continuar" : "Entrar Vacío"} <ArrowRight size={18} className="ml-2"/>
                                    </Button>
                                </div>
                            )}
                        </div>
                    ...
                    {step === 3 && (
                        <div className="animate-in zoom-in duration-500 text-center py-10">
                            ...
                            <p className="text-sys-500 text-lg mb-8 max-w-xs mx-auto">
                                Tu tienda <strong>{formData.businessName}</strong> ya está configurada y lista para vender.
                            </p>
                            ...
                            <div className="bg-sys-50 p-6 rounded-2xl border border-sys-200 mb-8 text-left space-y-3">
                                <div className="flex gap-3 items-center text-sys-700">
                                    <CheckCircle2 size={18} className="text-green-500" /> <span>Usuario Administrador Creado</span>
                                </div>
                                <div className="flex gap-3 items-center text-sys-700">
                                    <CheckCircle2 size={18} className="text-green-500" /> <span>Base de Datos Inicializada</span>
                                </div>
                                {selectedOption?.file && (
                                    <div className="flex gap-3 items-center text-sys-700">
                                        <CheckCircle2 size={18} className="text-green-500" /> <span>Catálogo Cargado</span>
                                    </div>
                                )}
                            </div>
