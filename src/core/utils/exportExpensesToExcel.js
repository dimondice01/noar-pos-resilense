...
const COLUMNS = [
    ...
    { key: 'description', label: 'Descripción', width: 40, numeric: false },
    { key: 'user',        label: 'Cajero',      width: 18, numeric: false },
    { key: 'type',        label: 'Tipo',        width: 12, numeric: false },
    { key: 'amount',      label: 'Monto ($)',   width: 15, numeric: true, numFmt: '"$"#,##0.00' },
];

const TYPE_LABELS = { EXPENSE: 'Gasto', OUT: 'Salida', WITHDRAWAL: 'Retiro' };

export const exportExpensesToExcel = async (items, filtersInfo = {}) => {
    ...
    const rows = items.map(m => ({
        ...
        user: m.user || 'Cajero',
        type: TYPE_LABELS[(m.type || '').toUpperCase()] || m.type || '-',
    ...
    const ws = workbook.addWorksheet('Detalle de Gastos');
    const BRAND = 'C2410C';
    ...
    parts.push(`Total: ${items.length} gastos · $ ${total.toLocaleString('es-AR', { minimumFractionDigits: 2 })}`);
    ...
    a.href = url;
    a.download = `detalle_gastos_${date}.xlsx`;
    a.click();
